...
check "httpgin 只依赖 app" internal/infra/httpgin 'infra/(repository|llm|embedding|pingcode|parser|config|database|crypto)|port|domain'
...
check "pingcode 不依赖 app"           internal/infra/pingcode   'app'
check "parser 不依赖 app"             internal/infra/parser     'app'
# 基建不感知业务
check "config 不依赖内部包"           internal/infra/config     '.*internal/(app|port|domain|infra/(repository|llm|embedding|pingcode|parser|httpgin))'
check "database 不依赖内部包"         internal/infra/database   '.*internal/(app|port|domain|infra/(repository|llm|embedding|pingcode|parser|httpgin))'
