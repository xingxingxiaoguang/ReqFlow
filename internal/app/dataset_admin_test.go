...
/* ---- 查重分析入参的 schema 注入（分析提示词按数据集口径） ---- */

func TestProfileWithInputSchemaOverridesTemplate(t *testing.T) {
	toy := model.DatasetSchema{Type: "review_note", Label: "评审发现", Fields: []model.FieldSpec{
		{Key: "finding", Label: "发现", Type: model.FieldString, Required: true, Prompt: "一句话概括"},
	}}
	profile, err := profileFor(model.TaskTypeRequirementImport)
	if err != nil {
		t.Fatalf("profileFor: %v", err)
	}
	overlaid := profileWithInputSchema(AnalyzeInput{SchemaJSON: marshalJSON(toy)}, profile)
	schema := overlaid.Schema()
	if schema.Type != "review_note" || len(schema.Fields) != 1 {
		t.Fatalf("数据集 schema 应覆写模板: %+v", schema)
	}
	// 写入工具校验同一口径
	if overlaid.Write.Schema.Type != "review_note" {
		t.Fatalf("写入绑定应同步覆写: %+v", overlaid.Write.Schema)
	}
	// 空载荷回退模板（不破坏旧行为）
	fallback := profileWithInputSchema(AnalyzeInput{}, profile)
	if fallback.Schema().Type != model.DatasetTypeRequirement {
		t.Fatalf("空载荷应回退任务类型模板: %+v", fallback.Schema())
	}
}

/* ---- 写入计划的 schema 解析方向（目标数据集自身） ---- */

func TestDatasetWritePlanResolvesFromTargetDataset(t *testing.T) {
	repo := newMemTasks()
	datasets := newMemDatasets()
	mgr := newTestManager(repo, &fakeParse{text: "x"}, nil, datasets, nil)
	// 数据集行上放玩具 schema（与任务类型 requirement_import 的模板不同）——
	// 写入按数据集自身的字段校验，任务类型不再决定字段
	target := seedTestDataset(t, datasets, toySchema(), "评审发现集")

	tk := &model.Task{Type: model.TaskTypeRequirementImport, OutputDatasetID: target.ID}
	plan, schema, err := mgr.datasetWritePlanFor(context.Background(), tk, DatasetTarget{Mode: WriteModeMerge, DatasetID: target.ID})
	if err != nil {
		t.Fatalf("datasetWritePlanFor: %v", err)
	}
	if plan.DatasetID != target.ID || plan.Mode != WriteModeMerge {
		t.Fatalf("plan = %+v", plan)
	}
	if schema.Type != toyDatasetType {
		t.Fatalf("schema 应来自目标数据集行（got %s，期望 %s）", schema.Type, toyDatasetType)
	}
}
