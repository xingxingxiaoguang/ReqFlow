...
type FieldSpec struct {
	...
	// = 运行时刻）。默认值随 schema 声明，不再散落归一化代码。
	Default any `json:"default,omitempty"`
	// Clean 值清洗声明（当前唯一取值 FieldCleanState：占位词/疑似 ID 视为未标注）。
...
// FieldCleanState Clean 声明取值：状态名清洗（空/占位词/疑似状态 ID → 视为未标注）。
const FieldCleanState = "state"
...
func RequirementSchema() DatasetSchema {
	return DatasetSchema{
		...
		Fields: []FieldSpec{
			...
			{Key: "state", Label: "状态", Type: FieldString, Filterable: true, Clean: FieldCleanState,
