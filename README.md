...
| 数字大脑 | 平台只读助手：解答平台操作问题、查询流程 / 任务 / 数据。内置三个 Skill——**平台指南**（/platform-guide）、**查询分析**（/query-analysis，多步搜索，召回不理想时提取原文关键词与关键语义重搜）、**创建 Skill**（/create-skill，引导用户把稳定工作方法沉淀为可复用的纯文本 Skill） |
| 归档管理 | 数据集归档与原样恢复 |
...
- 两条固定流程；示例知识库 **DH1功能&需求知识库** 四件套（字段结构「HD1-功能原文」、抽取规则「DH1文档数据清洗」、索引规则「HD1-语义索引」、空数据集）；数字大脑三个内置 Skill（平台指南、查询分析、创建 Skill）。
