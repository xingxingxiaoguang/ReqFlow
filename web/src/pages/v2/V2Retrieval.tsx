...
export default function V2Retrieval() {
  ...
  return <Space direction="vertical" size={16} style={{ width: '100%' }}>
    <Card title={<div><Title level={4} style={{ margin: 0 }}>混合检索工作台</Title><Paragraph type="secondary" style={{ margin: 0 }}>单次查询可调精准/语义权重、阈值、召回数和重排序 Top N；Profile 只负责不可变索引合同。</Paragraph></div>}>
      <Form form={form} layout="vertical" onFinish={search} initialValues={{ mode: 'hybrid', lexical_weight: 0.5, score_threshold: 0, recall_limit: 100, top_k: 20, rerank_enabled: true, rerank_top_n: 10 }}>
        ...
        <Row gutter={24}><Col xs={24} md={12}><Form.Item name="lexical_weight" label="精准搜索权重（其余为语义权重）"><Slider min={0} max={1} step={0.05} marks={{ 0: '纯语义', 0.5: '均衡', 1: '纯精准' }} /></Form.Item></Col><Col xs={24} md={12}><Form.Item name="score_threshold" label="召回分数阈值"><Slider min={0} max={1} step={0.05} /></Form.Item></Col></Row>
